...
6. **Key all metering state by `channelId`.** `acceptedCumulative` and the stored highest `SignedVoucher` MUST be keyed by the channel PDA (`channelId`), not by challenge id or HTTP session id. Multiple challenges that resume one channel share one cumulative ledger, so concurrent or cross-session reuse of one cumulative voucher cannot buy multiple responses.
