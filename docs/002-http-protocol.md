...
```json
{
  "voucher": {
    ...
    "openSlot": "<u64 decimal string; equals Channel.open_slot>",
...
7. **Authenticate resumed channels beyond decoding bytes.** When a challenge resumes an existing channel (`methodDetails.channelId`), the server MUST verify the decoded on-chain account is owned by the channel program and that its discriminator, version, status (`OPEN`), PDA derivation, mint (still allow-listed), `payee`, `authorized_signer`, `open_slot`, and `distribution_hash` all match the active challenge/session before metering against it. In particular, an `open_slot` that differs from the persisted value means the channel was closed and re-opened at the same seeds; the metering ledger MUST NOT be reused across that boundary — treat it as a distinct channel with a fresh `acceptedCumulative`.
8. **Gate PDA payees.** For PDA `payee` deployments, the server MUST either expose a tested CPI signer-seed `settleAndFinalize` adapter or reject the channel before metered service begins. Permissionless `settle` only applies while the channel is `OPEN` and cannot apply a voucher after the payer starts `requestClose`, so a PDA payee with no working cooperative-close path can leave delivered service uncollectible.
9. **Treat `Channel` state, not raw account balances, as authoritative.** The two addresses created during `open` are publicly derivable from the open payload — the **channel PDA** (`find_program_address([b"channel", payer, payee, mint, authorizedSigner, salt.to_le_bytes()], channelProgram)`) and the **escrow ATA** (`ATA(channelPda, mint, tokenProgram)`) — so a third party can prefund either before the server's `open` transaction lands. `open` is prefund-tolerant: both creates are idempotent, so a prefunded but still-uninitialized channel PDA (a system-owned, data-empty account that only holds lamports), or a pre-existing canonical escrow ATA, is accepted rather than reverting `open`. The server MUST use a `salt` unique per channel: reusing the `(payer, payee, mint, authorizedSigner, salt)` tuple of an already-open channel reverts `open`, because that PDA already holds an initialized `Channel` and is owned by the program, so the signed `Allocate` (which requires a system-owned, data-empty target) cannot re-run over it — resume that channel instead of re-opening it. Prefunded balances are never credited to the channel — surplus escrow tokens are swept to treasury at `finalize` and surplus PDA lamports refund to `rent_payer` when `distribute` closes the PDA on the FINALIZED branch — so the server MUST derive spendable capacity and pending settlement from `Channel` state (`deposit`, `settled`, `payout_watermark`), never from the raw escrow ATA balance or PDA lamports. See [ADR-001 → Accounting authority](./001-payment-channel-state-machine.md#accounting-authority).
...
sequenceDiagram
    ...
    S->>P: submit `distribute` ix (preimage)
    P->>P: verify hash(preimage)<br/>final cumulative deltas -> recipients/payee<br/>transfer (deposit − settled) → payer<br/>sweep irreducible residual → treasury<br/>close PDA (data → 0, lamports → rent_payer)<br/>state = closed
    P-->>S: OK
```
...
sequenceDiagram
    ...
    alt Cooperative — server responsive
        ...
        P->>P: verify hash(preimage)<br/>final cumulative deltas -> recipients/payee<br/>(if payerWithdrawnAt == 0) transfer (deposit − settled) → payer<br/>sweep irreducible residual → treasury<br/>close PDA (data → 0, lamports → rent_payer)<br/>state = closed
    end
```
