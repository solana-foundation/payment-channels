...
9. **Treat `Channel` state, not raw account balances, as authoritative.** The two addresses created during `open` are publicly derivable from the open payload — the **channel PDA** (`find_program_address([b"channel", payer, payee, mint, authorizedSigner, salt.to_le_bytes()], channelProgram)`) and the **escrow ATA** (`ATA(channelPda, mint, tokenProgram)`) — so a third party can prefund either before the server's `open` transaction lands. `open` is prefund-tolerant: both creates are idempotent, so a prefunded address no longer reverts `open` and the server does not need to retry with a fresh `salt`. Prefunded balances are never credited to the channel — surplus escrow tokens are swept to treasury at `finalize` and surplus PDA lamports refund to the payer at tombstone — so the server MUST derive spendable capacity and pending settlement from `Channel` state (`deposit`, `settled`, `payout_watermark`), never from the raw escrow ATA balance or PDA lamports. See [ADR-001 → Accounting authority](./001-payment-channel-state-machine.md#accounting-authority).
