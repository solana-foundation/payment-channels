...
- **Escape routes are direct-to-chain:** The client submits these directly to Solana RPC:
  ...
  - `distribute`: Permissionless in `OPEN` and `FINALIZED`. Caller supplies splits preimage; program verifies hash against `Channel.distribution_hash`. From `OPEN`, it pays recipients/payee only when floor math transfers the full `settled - paid_out` pool with no residual, then advances `paid_out` to `settled`. If flooring would leave any residual dust, it rejects without moving tokens or consuming the pool. From `FINALIZED`, it runs the final floored merchant payout, refunds `deposit - settled` to the payer (if `payerWithdrawnAt == 0`), sweeps final irreducible residual dust to treasury, and tombstones the PDA.
...
sequenceDiagram
    ...
    P->>P: verify hash(preimage)<br/>if floor math has no residual: transfer pool → recipients/payee<br/>paid_out += actual transferred amount<br/>otherwise reject without moving tokens<br/>state stays OPEN
    P-->>S: OK
    ...
    S->>P: submit `distribute` ix (preimage)
    P->>P: verify hash(preimage)<br/>final floored payout → recipients/payee<br/>transfer (deposit − settled) → payer<br/>sweep irreducible residual → treasury<br/>realloc to 1 byte<br/>state = tombstoned
    P-->>S: OK
```
...
sequenceDiagram
    ...
    alt Cooperative — server responsive
        ...
        P->>P: verify hash(preimage)<br/>final floored payout → recipients/payee<br/>(if payerWithdrawnAt == 0) transfer (deposit − settled) → payer<br/>sweep irreducible residual → treasury<br/>realloc to 1 byte<br/>state = tombstoned
    end
```
