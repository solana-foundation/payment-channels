...
3. **Mint allow-listing:** The server MUST restrict the channel `mint` to an explicit, server-controlled allow-list of vetted mints, and MUST reject any `POST /channel/open` whose decoded `mint` is not on that list — even when it otherwise matches the `402` challenge. The mint is fixed by the server in the challenge `currency` and re-validated against the decoded transaction in item 1, but the on-chain program does not inspect a mint's freeze or mint authority (see ADR-001, *Mint trust model*): a live freeze authority on the escrowed mint can permanently freeze the channel escrow and every payout/refund leg, with no permissionless crank to unwind it. Because the program treats the mint as a trust dependency rather than a parameter it defends against, the server is the unbypassable gate that keeps arbitrary payer-chosen freezeable mints out of channels. The server MUST curate this allow-list out of band — admitting only mints whose freeze and mint authorities it considers acceptably governed (e.g. reputable issuer stablecoins) — and MUST NOT derive it from client-supplied data.
