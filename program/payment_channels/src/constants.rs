...
/// builds; a `devnet`/`mainnet` build rejects it (gate below), forcing a real
/// owner.
...
/// derivations consistent in tests. A `devnet`/`mainnet` build rejects it (gate
/// below). The anti-cross-cluster-replay property only matters on real clusters.
...
// mainnet > devnet > localnet/default). To onboard a new per-cluster value, add
// it to each block. `CHAIN_ID` is the cluster's genesis hash — Solana's
// canonical chain identifier (the basis of CAIP-2 `solana:<genesis-hash
// prefix>`); a program has no runtime access to it, so it is compiled in. Set
// the devnet/mainnet `TREASURY_OWNER` to the real owner before deploy, e.g.
// `const_crypto::bs58::decode_pubkey("Your…Owner")`.
#[cfg(feature = "mainnet")]
mod cluster {
    use super::*;
    pub const TREASURY_OWNER: [u8; 32] = TREASURY_OWNER_SENTINEL; // TODO: real mainnet owner
...
/// Build-time guard: a devnet/mainnet build must ship neither placeholder.
...
const _: () = assert!(
    ...
    "CHAIN_ID is still the localnet placeholder; a devnet/mainnet build must use \
     that cluster's genesis hash",
);
